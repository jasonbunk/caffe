#ifndef CAFFE_ONE_VERSUS_ALL_LOSS_LAYER_HPP_
#define CAFFE_ONE_VERSUS_ALL_LOSS_LAYER_HPP_
...
/**
 * @brief Computes one-versus-all binarized loss function, useful for
  *        binary classification or multilabel classification.
 *
...
class OneVersusAllLossLayer : public LossLayer<Dtype> {
 public:
  explicit OneVersusAllLossLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "OneVersusAllLoss"; }

 protected:
  /// @copydoc OneVersusAllLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  /**
   * @brief Computes a sigmoid one-versus-all loss error gradient w.r.t. the
   *        predictions.
   *
  ...
  int ignore_label_;
  /// should we apply sigmoid to the input?
  bool do_sigmoid_;
...
#endif  // CAFFE_ONE_VERSUS_ALL_LOSS_LAYER_HPP_
